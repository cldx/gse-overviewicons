...
// Remastered 2013 by Joern Konopka <cldx3000[at]googlemail.com>
...
const WINDOWOVERLAY_ICON_SIZE = 48;
...
function enable() {
    ...
    wsWinOverInjections['_init'] = injectToFunction(Workspace.WindowOverlay.prototype, '_init', function(windowClone, parentActor) {
        ...
        this._applicationIconBox = new St.Bin({ style_class: 'windowoverlay-icon-box-wrapper' });
	this._iconBoxWrapper = new St.Bin({ style_class: 'windowoverlay-application-icon-box' });
        this._applicationIconBox.set_opacity(255);
	this._iconBoxWrapper.add_actor(icon);
        this._applicationIconBox.add_actor(this._iconBoxWrapper);
    ...
    wsWinOverInjections['_onEnter'] = injectToFunction(Workspace.WindowOverlay.prototype, '_onEnter', function() {
		this._applicationIconBox.add_style_pseudo_class('hover');
    ...
    wsWinOverInjections['_onLeave'] = injectToFunction(Workspace.WindowOverlay.prototype, '_onLeave', function() {
        this._applicationIconBox.remove_style_pseudo_class('hover');
       /* Tweener.addTween(this._applicationIconBox, { time: 0.2,
                                                     opacity: 255,
                                                     transition: 'linear' }); */
    });
	...
	let iconX = cloneX - Math.floor(icon.width/4);
	
	let iconY = cloneY - Math.floor(icon.width/4);	        
